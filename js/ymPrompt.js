...
(function() {
	...
	/*创建按钮*/
	var defaultBtn=function(){return {OK:[curCfg.okTxt, 'ok'], CANCEL:[curCfg.cancelTxt, 'cancel'], CLOSE:[curCfg.closeTxt, 'close']}};
	var mkBtn = function(txt, sign, autoClose, id) {
		if (!txt) return;
		if (txt instanceof Array) {
			/*无效按钮删除*/
			if(txt.length>1)$('hintIcon').className="ques";
			else $('hintIcon').className="";
			var item,t=[];
		...
		if(sign!='cancel'){
			return {
				id: id,
				html: "<button type='button' class='btn btn-info' id='" + id + "' onclick='ymPrompt.doHandler(\"" + sign + "\"," + autoClose + ")'>"+txt+"</button>"
			};
		}else{
			return {
				id: id,
				html: "<a href='javascript:;' class='btn B1'id='" + id + "' onclick='ymPrompt.doHandler(\"" + sign + "\"," + autoClose + ")'>"+txt+"</a>"
			};
		}
	...
	(function() {
		...
		/*div.innerHTML = [
		/!*遮罩*!/
		"<div id='maskLevel' style=\'" + maskStyle + ';z-index:10000000;\'></div>', useIframe ? ("<iframe id='maskIframe' style='" + maskStyle + ";z-index:9999999;filter:alpha(opacity=0);opacity:0'></iframe>") : '',
		/!*窗体*!/
		"<div id='hint' style='position:absolute;z-index:10000001;display:none' class='float'>", useIframe ? "<iframe style='width:100%;height:100%;position:absolute;top:0;left:0;z-index:-1'></iframe>": '', "<div class='section fHead' id='ym-tl'><a href='javascript:;' onclick='ymPrompt.doHandler(\"close\")'></a><p>温馨提示</p></div><div class='section fBody'>", "<div class='hintCon' id='ym-ml'><div id='hintIcon' class='ques'></div><p></p></div><p class='hr'></p>", "<div class='inputBig' id='ym-btnl'><button>确 定</button><a href='javascript:;' class='btn B1'>取 消</a></div>", "<div class='ym-bl' id='ym-bl'><div class='ym-br'><div class='ym-bc'></div></div></div>", "</div>"].join('');
		...
		var SetWinSize = function(w, h) {
			...
			var topheight=jQuery(window.document).scrollTop();
			if(top!=self){
				topheight=topheight+20;
			}else{
				topheight=topheight+80;
			}
			setStyle(ym_win, {
				top: (topheight) + 'px',
				left: pos[0] + 'px'
		...
		var winVisible = function(visible) {
			...
			ym_hTool.innerHTML = curCfg.title; //标题ym_body
			//ym_hTool.innerHTML = "<div class='ymPrompt_close' title='"+curCfg.closeTxt+"' onclick='ymPrompt.doHandler(\"close\")'>&nbsp;</div>";
			if(curCfg.message.length<13){
				$('ym-ml').className="hintCon single";
			}else{
				$('ym-ml').className="hintCon";
			}
			ym_body.innerHTML = !curCfg.iframe ? (curCfg.message) : "<iframe width='100%' height='100%' border='0' frameborder='0' src='" + curCfg.message + "'></iframe>"; //内容
		...
		ymPrompt.apply(ymPrompt, {
			...
			show: function(args, fargs) {
				...
				if (typeof a[0] != 'object') {
					var cfg = ['message', 'width', 'height', 'title', 'handler', 'maskAlphaColor', 'maskAlpha', 'iframe', 'icoCls', 'btn', 'autoClose', 'fixPosition', 'dragOut', 'titleBar', 'showMask', 'winPos','okTxt','cancelTxt'];
					for (var i = 0,l = a.length; i < l; i++) if (a[i]!=null&&a[i]!='undefined') o[cfg[i]] = a[i];
				} else {
					o = a[0];
				}
